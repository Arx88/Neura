...
async def run_agent(
    ...
    # current_task_id_for_run is removed from here; current_task_id is now sourced from TaskPlanner
    # final_planned_task_id_for_execution is also removed as current_task_id will serve this.
    current_task_id: Optional[str] = None # Will be set by TaskPlanner's result
    ...
    try:
        ...
        await _add_task_log_message(task_state_manager, thread_id, f"Agent run initiated for thread_id {{thread_id}} with prompt: {{initial_prompt_text[:70]}}...", log_type="info")
        ...
        if not planned_main_task_obj or planned_main_task_obj.status == "planning_failed":
            ...
            logger.error(f"Planning phase failed for thread_id {{thread_id}}. Error: {{error_msg}}")
            task_id_for_planning_failure_log = planned_main_task_obj.id if planned_main_task_obj else thread_id
            try:
               ...
               if planned_main_task_obj:
                    await task_state_manager.fail_task(task_id=planned_main_task_obj.id, error=error_msg, progress=0.1)
        ...
        current_task_id = planned_main_task_obj.id
        logger.info(f"Planning completed by TaskPlanner. Main plan task ID: {{current_task_id}} for original thread_id {{thread_id}}.")
        await _add_task_log_message(task_state_manager, current_task_id, f"Phase 1 (Planning) completed. Model: {{model_name}}. Prompt: {{initial_prompt_text[:70]}}...")
        await task_state_manager.set_task_status(task_id=current_task_id, status="executing", progress=planned_main_task_obj.progress or 0.15)

        # --- Execution Phase ---
        await _add_task_log_message(task_state_manager, current_task_id, "Phase 2: Execution starting.")
        ...
        await plan_executor.execute_plan_for_task(current_task_id)

        logger.info(f"Plan execution completed for task {{current_task_id}}")
        await _add_task_log_message(task_state_manager, current_task_id, "Plan execution phase concluded by PlanExecutor.")

        final_task_state = await task_state_manager.get_task(current_task_id)
        if final_task_state and final_task_state.status not in ["failed", "completed"]:
            logger.warning(f"Main plan task {{current_task_id}} ended in status '{{final_task_state.status}}' after PlanExecutor. Forcing complete.")
            await task_state_manager.complete_task(task_id=current_task_id, result={{"summary": "Task execution phase concluded by run_agent wrapper."}}, progress=1.0)
        ...
        task_id_to_report_failure_on = None
        if 'current_task_id' in locals() and locals()['current_task_id']:
            task_id_to_report_failure_on = locals()['current_task_id']

        if task_id_to_report_failure_on:
            try:
            ...
            logger.error(f"Orchestration error for thread_id {{thread_id}} with no task_id established. Prompt: {{initial_prompt_text[:70] if 'initial_prompt_text' in locals() else 'N/A'}} Error: {{error_summary}}")
